...
uint8_t getEstimatedAltitude(){
	...
	// Not needed for Sonar Based Altitude Hold
	//  if(calibratingB > 0) {
	...
	//BaroAlt not needed for Sonar based Altitude Hold
	//BaroAlt = log( baroGroundPressure * (BARO_TAB_SIZE - 1)/ (float)baroPressureSum ) * (baroTemperature+27315) * 29.271267f; // in cemtimeter

	//EstAlt = (EstAlt * 6 +  * 2) >> 3; // additional LPF to reduce sonar noise (faster by 30 µs)
	// Default : EstAlt = (EstAlt * 6 + BaroAlt * 2) >> 3; // additional LPF to reduce baro noise (faster by 30 µs)
	// Changes made above to implement to sonar based altitude hold instead of Barometer
	
	
	//debug[2] = rcData[AUX1]; 
	//sonarAlt = sonar.getDistance();
	
	EstAlt=Filter(sonarAlt,EstAlt,10);

	#if (defined(VARIOMETER) && (VARIOMETER != 2)) || !defined(SUPPRESS_BARO_ALTHOLD)
	//P
	int16_t error16 = constrain(AltHold - EstAlt, -60, 60);                                 //    int16_t error16 = constrain(AltHold - EstAlt, -300, 300);
	applyDeadband(error16, 10); //remove small P parametr to reduce noise near zero position
	BaroPID = constrain((conf.P8[PIDALT] * error16 >>4), -150, +150);                       //BaroPID = constrain((conf.P8[PIDALT] * error16 >>7), -150, +150);
	...
	errorAltitudeI += conf.I8[PIDALT] * error16 >>6;
	errorAltitudeI = constrain(errorAltitudeI,-150,150);
	BaroPID += errorAltitudeI>>6; //I in range +/-60
